...
namespace glm {
	...
	/// Compute a covariance matrix form a pair of iterators `b` (begin) and `e` (end) of a container with relative coordinates (e.g., relative to the
	/// center of gravity of the object) Dereferencing an iterator of type I must yield a `vec&lt;D, T, Q%gt;`
	...
	/// Compute a covariance matrix form a pair of iterators `b` (begin) and `e` (end) of a container with absolute coordinates and a precomputed
	/// center of gravity `c` Dereferencing an iterator of type I must yield a `vec&lt;D, T, Q%gt;`
	template<length_t D, typename T, qualifier Q, typename I>
	GLM_FUNC_DECL mat<D, D, T, Q> computeCovarianceMatrix(I const& b, I const& e, vec<D, T, Q> const& c);

	/// Assuming the provided covariance matrix `covarMat` is symmetric and real-valued, this function find the `D` Eigenvalues of the matrix, and
	/// also provides the corresponding Eigenvectors. Note: the data in `outEigenvalues` and `outEigenvectors` are in matching order, i.e.
	/// `outEigenvector[i]` is the Eigenvector of the Eigenvalue `outEigenvalue[i]`. This is a numeric implementation to find the Eigenvalues, using
	/// 'QL decomposition` (variant of QR decomposition: https://en.wikipedia.org/wiki/QR_decomposition).
	///
	...
	GLM_FUNC_DECL unsigned int findEigenvaluesSymReal(
	    mat<D, D, T, Q> const& covarMat, vec<D, T, Q>& outEigenvalues, mat<D, D, T, Q>& outEigenvectors
	);

	/// Sorts a group of Eigenvalues&Eigenvectors, for largest Eigenvalue to smallest Eigenvalue.
	/// The data in `outEigenvalues` and `outEigenvectors` are assumed to be matching order, i.e. `outEigenvector[i]` is the Eigenvector of the
	/// Eigenvalue `outEigenvalue[i]`.
